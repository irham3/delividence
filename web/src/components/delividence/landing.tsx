...
function SourceListPanel() { return <Panel><PanelHeader>SOURCES</PanelHeader><div>{sourceFiles.map(([name, time, kind]) => { const Icon = kind === "mail" ? Mail : kind === "audio" ? AudioLines : kind === "image" ? ImageIcon : kind === "video" ? Video : FileText; return <div key={name} className="grid min-h-[66px] grid-cols-[32px_1fr_20px] items-center gap-3 border-b border-[#dedad1] px-4"><span className="flex h-8 w-8 items-center justify-center bg-[#e9e6df]"><Icon size={15} strokeWidth={1.5} /></span><div className="min-w-0"><p className="truncate text-[12px]">{name}</p><p className="mt-1 text-xs text-[#6b6c65]">{time}</p></div><ChevronRight size={15} className="lg:hidden" /></div>; })}<button className="tap focus-ring flex min-h-12 w-full items-center gap-3 px-4 text-[12px]"><Plus size={16} /> Add source</button></div></Panel>; }
