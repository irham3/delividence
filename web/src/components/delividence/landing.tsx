    ...
    () => {
      ...
      if (reducedMotion) {
        ...
        gsap.set(
          ...
          { autoAlpha: 1, scaleX: 1, scaleY: 1, x: 0, y: 0 },
        );
    ...
    <main ref={root} className="paper-texture landing-canvas min-h-[100dvh] overflow-x-clip">
      ...
      <header className="site-header fixed inset-x-0 top-0 z-40 px-5 sm:px-8">
        <div className="site-header-inner mx-auto flex h-[72px] max-w-[1344px] items-center justify-between border-b border-[#a9a69f] sm:h-[78px]">
          <a href="#top" className="focus-ring text-[17px] font-semibold text-[#171815] sm:text-[18px]">Delividence</a>
          <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-7 text-[12px] font-medium text-[#343530] md:flex lg:gap-9 lg:text-[13px]" aria-label="Primary navigation">
            <a className="focus-ring hover:text-[#11120f]" href="#workflow">How it works</a>
            <a className="focus-ring hover:text-[#11120f]" href="#clarification">Clarify the brief</a>
            <a className="focus-ring hover:text-[#11120f]" href="#changes">Protect the baseline</a>
            <a className="focus-ring hover:text-[#11120f]" href="#review">Review delivery</a>
            <a className="focus-ring hover:text-[#11120f]" href="#about">About Delividence</a>
          </nav>
        ...
        {menuOpen && <nav aria-label="Mobile navigation" className="mx-auto max-w-[1344px] border-b border-[#a9a69f] bg-[#f4f1ea]/95 px-4 py-4 backdrop-blur-sm md:hidden"><div className="grid gap-1"><a className="tap focus-ring px-2 py-3 text-sm font-medium" href="#workflow" onClick={() => setMenuOpen(false)}>How it works</a><a className="tap focus-ring px-2 py-3 text-sm font-medium" href="#clarification" onClick={() => setMenuOpen(false)}>Clarify the brief</a><a className="tap focus-ring px-2 py-3 text-sm font-medium" href="#changes" onClick={() => setMenuOpen(false)}>Protect the baseline</a><a className="tap focus-ring px-2 py-3 text-sm font-medium" href="#review" onClick={() => setMenuOpen(false)}>Review delivery</a><a className="tap focus-ring px-2 py-3 text-sm font-medium" href="#about" onClick={() => setMenuOpen(false)}>About Delividence</a><a className="tap focus-ring px-2 py-3 text-sm font-medium" href="/sign-in">Sign in</a></div></nav>}
      </header>

      <section id="top" className="hero-stage mx-auto grid min-h-[100dvh] max-w-[1344px] items-center gap-12 px-5 pb-14 pt-[112px] sm:px-8 sm:pb-14 sm:pt-[126px] lg:grid-cols-[minmax(320px,0.58fr)_minmax(0,1fr)] lg:px-8 lg:pb-10 lg:pt-[98px] xl:px-0">
        <div className="hero-copy relative z-10 self-center lg:pb-3">
          <p className="mono text-xs font-semibold text-[#8d5900]">1. Collect the material.</p>
          <p className="handwritten mt-5 max-w-[175px] border-l border-[#c98a17] pl-[14px] text-[21px] font-medium leading-[1.34] text-[#ba7807]">One place for<br />the material<br />behind the work</p>
          <h1 className="mt-10 max-w-[8.8ch] text-[48px] font-medium leading-[1.02] text-[#171815] sm:mt-[46px] sm:text-[62px] lg:text-[70px] xl:text-[80px]">The brief is<br />more than<br />the brief.</h1>
          <p className="mt-6 max-w-[425px] text-[16px] leading-[1.65] text-[#595a55] sm:mt-7 sm:text-[17px]">Emails, notes, images, and calls become a record the project can actually use.</p>
      ...
      <section id="workflow" className="material-stage editorial-section relative mx-auto max-w-[1344px] scroll-mt-20 px-5 py-20 sm:px-8 sm:py-24 lg:px-8 lg:py-28 xl:px-0 xl:py-32">
        <SectionHeading number="2" title="Turn scattered material into a working record." />
        <div className="mt-9 border-y border-[#c9c5bc] lg:mt-12 lg:grid lg:grid-cols-[72%_28%]">
          <div className="border-[#c9c5bc] py-5 lg:border-r lg:px-5 lg:py-6">
            <div className="flex items-center gap-5"><p className="mono shrink-0 text-xs font-semibold">SOURCES</p><span className="source-progress h-px flex-1 bg-[#565750]" /><ChevronRight className="source-arrow" size={18} strokeWidth={1.25} /></div>
          ...
          <div className="project-record-panel relative py-6 lg:p-6">
            ...
            <p className="mono text-xs font-semibold">PROJECT RECORD</p>
            <h3 className="mt-7 text-[18px] font-semibold">Project Agreement (v1)</h3>
            <ul className="mt-6 space-y-4 pr-7 text-[14px] leading-6 text-[#3f403b]">{agreementItems.map((item) => <li key={item}>{item}</li>)}</ul>
            <p className="mono mt-7 text-xs text-[#555650]">Date: Apr 29 · Owner: Design Team</p>
          </div>
        </div>
        <JourneyConnector label="Record assembled" />
      </section>

      <section id="clarification" className="editorial-section relative mx-auto max-w-[1344px] scroll-mt-20 px-5 py-20 sm:px-8 sm:py-24 lg:px-8 lg:py-28 xl:px-0 xl:py-32">
        <SectionHeading number="3" title="Resolve only what the record cannot answer." />
      ...
      <section id="changes" className="decision-stage editorial-section relative mx-auto max-w-[1344px] scroll-mt-20 px-5 py-20 sm:px-8 sm:py-24 lg:px-8 lg:py-28 xl:px-0 xl:py-32">
        <SectionHeading number="4" title="Protect the baseline when work changes." />
      ...
      <section id="review" className="proof-stage editorial-section relative mx-auto max-w-[1344px] scroll-mt-20 px-5 py-20 sm:px-8 sm:py-24 lg:px-8 lg:py-28 xl:px-0 xl:py-32">
        <SectionHeading number="5" title="Review proof against what was agreed." />
      ...
      <section id="about" className="editorial-section relative mx-auto max-w-[1344px] scroll-mt-20 px-5 pb-20 pt-20 sm:px-8 sm:pb-24 sm:pt-24 lg:px-8 lg:pb-28 lg:pt-28 xl:px-0 xl:pt-32">
        ...
        <div className="closing-cta mx-auto mt-20 max-w-3xl text-center sm:mt-24"><h2 className="text-[34px] font-medium leading-[1.12] sm:text-[42px] lg:text-[48px]">Give the work a record worth returning to.</h2><button className="tap focus-ring mt-8 min-h-12 w-full rounded-[4px] bg-[#8d5900] px-8 py-3.5 text-[14px] font-semibold text-white shadow-[0_3px_10px_rgba(120,76,0,.16)] hover:bg-[#744900] sm:w-auto" onClick={onRegister}>Create a record</button></div>
      </section>

      <footer className="border-t border-[#bcb8af] px-5 py-8 sm:px-8"><div className="mx-auto grid max-w-[1344px] gap-6 text-[13px] text-[#5f605a] md:grid-cols-3 md:items-center"><span>© 2026 Delividence</span><p className="flex flex-wrap gap-x-8 gap-y-3 md:justify-center"><span>Security</span><span>Privacy</span><span>Terms</span></p><a className="focus-ring hover:text-[#171815] md:text-right" href="mailto:hello@delividence.com">hello@delividence.com</a></div></footer>
    </main>
  );
}

function SectionHeading({ number, title }: { number: string; title: string }) { return <div className="section-heading relative flex items-start gap-4 pb-4 sm:gap-6"><span className="section-number text-[22px] font-medium leading-none sm:text-[24px]">{number}.</span><h2 className="section-title text-[26px] font-medium leading-[1.05] sm:text-[32px] lg:text-[36px]">{title}</h2><span className="section-heading-rule absolute inset-x-0 bottom-0 h-px bg-[#bcb8af]" aria-hidden="true" /></div>; }

function JourneyConnector({ label }: { label: string }) {
  return <div className="journey-connector pointer-events-none absolute -bottom-14 left-1/2 z-20 h-28 w-44 -translate-x-1/2" aria-hidden="true"><span className="absolute left-1/2 top-0 h-[86px] w-px bg-[#2f302b]/15" /><span className="journey-progress absolute left-1/2 top-0 h-[86px] w-px origin-top bg-[#c98a17]" /><ArrowDown className="journey-arrow absolute left-1/2 top-0 -translate-x-1/2 bg-[#f4f1ea] text-[#b57900]" size={20} strokeWidth={1.35} /><span className="journey-label mono absolute left-[calc(50%+20px)] top-[43px] hidden whitespace-nowrap text-[10px] uppercase text-[#77786f] sm:block">{label}</span></div>;
}
...
function HeroArtifacts() {
  return <div className="hero-art-board w-full">
    <div className="grid grid-cols-2 gap-3 xl:hidden">
      ...
      <article className="hero-artifact hero-document p-3"><p className="mono text-[8px]">HERO v2</p><p className="mt-4 text-[20px] font-semibold leading-[1.05]">Work that<br />moves people.</p><p className="mt-3 text-[9px] leading-4">Strategy, design, and brand craftsmanship.</p></article>
    ...
    <div className="relative hidden min-h-[570px] w-full xl:block">
      ...
      <HeroIndexMarker label="S-01" className="left-[-1.5%] top-[112px]" /><HeroIndexMarker label="S-02" className="left-[-2.5%] top-[374px]" />
    </div>
  </div>;
}
...
function MaterialSourceCard({ kind, label, caption }: { kind: "brief" | "email" | "call" | "screenshot" | "video"; label: string; caption: string }) {
  return <article className="scene-two-source reveal-item w-[172px] shrink-0 snap-start lg:w-auto"><p className="mono mb-3 text-xs font-semibold">{label}</p><div className="hero-document relative h-[260px] overflow-hidden p-4 lg:h-[238px] lg:p-3">
...
function PanelHeader({ children }: { children: React.ReactNode }) { return <div className="flex min-h-12 items-center justify-between border-b border-[#d4d0c7] px-4"><p className="mono text-xs font-semibold">{children}</p><span className="hidden text-[16px] lg:block">⌃</span></div>; }

function SourceListPanel() { return <Panel><PanelHeader>SOURCES</PanelHeader><div>{sourceFiles.map(([name, time, kind]) => { const Icon = kind === "mail" ? Mail : kind === "audio" ? AudioLines : kind === "image" ? ImageIcon : kind === "video" ? Video : FileText; return <div key={name} className="grid min-h-[66px] grid-cols-[32px_1fr_20px] items-center gap-3 border-b border-[#dedad1] px-4"><span className="flex h-8 w-8 items-center justify-center bg-[#e9e6df]"><Icon size={15} strokeWidth={1.5} /></span><div className="min-w-0"><p className="truncate text-[12px]">{name}</p><p className="mt-1 text-xs text-[#6b6c65]">{time}</p></div><ChevronRight size={15} className="lg:hidden" /></div>; })}<div className="flex min-h-12 w-full items-center gap-3 px-4 text-[12px]"><Plus size={16} /> Add source</div></div></Panel>; }

function ExtractedPlanPanel() { return <Panel><PanelHeader>EXTRACTED PLAN</PanelHeader><div className="px-5 py-5"><h3 className="text-[15px] font-semibold">Project Agreement (v1)</h3><ul className="mt-5 space-y-3 text-[13px] leading-5 text-[#3f403b]">{agreementItems.map((item) => <li key={item}>{item}</li>)}</ul><p className="mono mt-6 text-xs text-[#77786f]">Last updated: Apr 29, 10:11 AM</p></div><div className="border-t border-[#d4d0c7] px-5 py-4"><p className="mono text-xs font-semibold">LINKED EVIDENCE</p><div className="mt-3 flex flex-wrap gap-2">{[1, 2, 3, 4, 5].map((n) => <span key={n} className="border border-[#bdb9b0] px-3 py-1.5 text-xs">S-0{n}</span>)}</div><a href="#workflow" className="focus-ring mt-4 inline-block border-b border-[#292a26] text-xs">View all sources</a></div></Panel>; }

function QuestionsPanel() { const questions = ["What is the final duration for the hero video?", "Should the button have a secondary style?", "Do we need a mobile specific layout?"]; return <Panel><PanelHeader>UNRESOLVED QUESTIONS</PanelHeader><div className="space-y-3 p-4">{questions.map((q, i) => <div key={q} className="grid min-h-[76px] w-full grid-cols-[24px_1fr_18px] items-start gap-2 border border-[#d4d0c7] p-4 text-left text-[12px] leading-5"><b>{i + 1}</b><span>{q}</span><ChevronRight size={16} className="lg:hidden" /></div>)}</div></Panel>; }

function ClientReviewPanel() { return <Panel><PanelHeader>CLIENT REVIEW</PanelHeader><div className="p-4"><label className="text-xs text-[#666760]" htmlFor="review-link">Secure review link</label><div className="mt-2 flex h-10 items-center border border-[#aaa69d] px-3"><input id="review-link" readOnly value="delividence.com/r/8f3k2a" className="min-w-0 flex-1 bg-transparent text-[12px] outline-none" /><Copy size={14} /></div><p className="mono mt-6 text-xs text-[#666760]">Client reply · May 1, 11:08 AM</p><ol className="mt-4 space-y-3 text-[12px] leading-5"><li><b className="mr-4">1</b>20 seconds.</li><li><b className="mr-4">2</b>Yes, outline style.</li><li className="flex"><b className="mr-4">3</b><span>Yes, mobile layout is required.</span></li></ol></div><div className="flex items-center gap-3 border-t border-[#d4d0c7] px-4 py-4 text-xs"><Check size={15} strokeWidth={1.5} /><span>Plan updated<br /><span className="text-[#77786f]">May 1, 11:09 AM</span></span></div></Panel>; }
...
function DecisionPaper({ eyebrow, variant, children }: { eyebrow: string; variant: "ruled" | "grid" | "decision"; children: React.ReactNode }) { return <article className="decision-paper-wrap relative"><p className="mono mb-4 text-xs font-semibold">{eyebrow}</p><div className={`decision-paper paper-card relative min-h-[390px] p-6 sm:p-8 ${variant === "ruled" ? "paper-ruled" : variant === "grid" ? "paper-grid" : "paper-decision"}`}>{children}</div></article>; }

function ProofCriteria() { const criteria = ["Header simplification", "Video starts muted", "Button label", "Testimonials section", "Mobile layout"]; return <article className="proof-criteria border-[#c9c5bc] py-5 lg:border-r lg:p-5"><p className="mono text-xs font-semibold">PROOF: Homepage v2</p><p className="mt-1 text-xs text-[#77786f]">Delivered: May 5, 4:42 PM</p><div className="mt-5 grid grid-cols-[1fr_auto] border-y border-[#d4d0c7] py-3 text-xs text-[#77786f]"><span>Criterion</span><span>Status</span></div><div>{criteria.map((criterion) => <div key={criterion} className="grid min-h-10 grid-cols-[1fr_auto] items-center text-xs"><span>{criterion}</span><span className="criteria-check flex items-center gap-1.5 text-[#2d7552]"><Check size={13} /> Met</span></div>)}</div></article>; }

function EvidencePanel() { return <article className="evidence-panel min-w-0 border-[#c9c5bc] py-5 lg:border-r lg:p-5"><div className="flex items-center gap-4"><p className="mono shrink-0 text-xs font-semibold">EVIDENCE</p><span className="proof-route h-px flex-1 bg-[#c98a17]" aria-hidden="true" /><ChevronRight className="proof-arrow text-[#b57900]" size={17} strokeWidth={1.4} aria-hidden="true" /></div><div className="editorial-scroll -mx-5 mt-5 overflow-x-auto px-5 pb-3 lg:mx-0 lg:px-0"><div className="evidence-strip flex w-max gap-3"><EvidenceCard type="screen" file="hero_v2_final.png" /><EvidenceCard type="video" file="hero_v2_final.mp4" /><EvidenceCard type="mobile" file="mobile_v2.png" /><EvidenceCard type="note" file="testimonials_v2.png" /></div></div><div className="mt-4 border-t border-[#d4d0c7] pt-4"><p className="mono text-xs font-semibold">LIVE URL</p><span className="mt-1 inline-flex items-center gap-2 border-b border-[#292a26] text-[12px]">https://deliv.co/ <ExternalLink size={13} /></span></div></article>; }

function EvidenceCard({ type, file }: { type: "screen" | "video" | "mobile" | "note"; file: string }) { return <figure className="w-[142px] shrink-0 snap-start"><div className="evidence-capture relative h-[154px] overflow-hidden p-3">{type === "screen" && <><p className="text-[15px] font-semibold leading-none">Work that<br />moves people.</p><p className="mt-3 text-[7px]">Strategy, design, and brand craftsmanship.</p><div className="absolute bottom-2 left-2 right-2 h-[66px] overflow-hidden rounded-[2px]"><Image src="/assets/hero-coast.png" alt="Final homepage screenshot" fill sizes="130px" className="object-cover" /></div></>}{type === "video" && <><Image src="/assets/hero-coast.png" alt="Final homepage video" fill sizes="142px" className="object-cover" /><Play className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-white" fill="currentColor" /><span className="absolute bottom-2 right-2 text-[9px] text-white">00:20</span></>}{type === "mobile" && <div className="mx-auto h-full w-[66px] rounded-[12px] border-[3px] border-[#1d1e1b] bg-[#f4f1ea] p-1"><div className="h-2 border-b border-[#bbb7ae]" /><p className="mt-2 text-[7px] font-bold">Work that<br />moves people.</p><div className="relative mt-2 h-16 overflow-hidden rounded-[2px]"><Image src="/assets/hero-coast.png" alt="Mobile capture" fill sizes="60px" className="object-cover" /></div></div>}{type === "note" && <><p className="mono text-center text-[8px]">Client note</p><div className="mt-5 space-y-5">{[1, 2].map((n) => <div key={n} className="flex gap-2"><span className="h-5 w-5 rounded-full bg-[#d6d2c9]" /><div className="flex-1 space-y-1"><span className="block h-px bg-[#77786f]/50" /><span className="block h-px w-4/5 bg-[#77786f]/35" /><span className="block h-px w-3/5 bg-[#77786f]/35" /></div></div>)}</div></>}</div><figcaption className="mono mt-2 truncate text-xs text-[#666760]">{file}</figcaption></figure>; }

function ClientDecisionPanel() { return <article className="client-decision py-5 lg:p-5"><p className="mono text-xs font-semibold">CLIENT DECISION</p><p className="mt-6 text-[12px]">This delivers what was agreed.</p><div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-1"><div className="flex min-h-11 items-center justify-center gap-2 border border-[#2d7552] text-[12px] text-[#2d7552]"><Check size={14} /> Accept</div><div className="grid min-h-11 place-items-center border border-[#aaa69d] text-[12px]">Request changes</div><div className="grid min-h-11 place-items-center border border-[#aaa69d] text-[12px] sm:col-span-2 lg:col-span-1">Not accept</div></div><p className="mt-6 block text-xs text-[#77786f]">Add comment (optional)</p><div className="mt-2 min-h-24 w-full border border-[#c9c5bc] bg-transparent p-3 text-[12px]" /></article>; }
